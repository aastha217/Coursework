...
WITH ordered AS (
    -- 1️⃣ Order each student's study sessions by date, and get the previous session date
    SELECT
        ss.*,
...
marked AS (
    -- 2️⃣ Mark whether there is a break in the sequence (>2 days gap starts a new segment)
    SELECT
        *,
        CASE
...
segmented AS (
    -- 3️⃣ Assign a segment ID to each continuous segment using a cumulative sum
    SELECT
        *,
        SUM(is_break) OVER (
            PARTITION BY student_id
...
segment_stats AS (
    -- 4️⃣ Aggregate statistics at the segment level
    SELECT
        student_id,
        segment_id,
        COUNT(*) AS session_cnt,                 -- number of sessions in this segment
        COUNT(DISTINCT subject) AS cycle_length, -- number of distinct subjects in this segment
        SUM(hours_studied) AS total_study_hours -- total hours studied in this segment
    FROM segmented
    GROUP BY student_id, segment_id
),

valid_segments AS (
    -- 5️⃣ Filter segments that meet the "Study Spiral Pattern" criteria
    SELECT *
    FROM segment_stats
    WHERE
        cycle_length >= 3          -- at least 3 different subjects
        AND session_cnt >= 2 * cycle_length   -- at least 2 complete cycles
)

-- 6️⃣ Join with the students table to get student details, and output the result
SELECT
    v.student_id,
    s.student_name,
    s.major,
...
ORDER BY
    v.cycle_length DESC,        -- first order by cycle length descending
    v.total_study_hours DESC;   -- then by total study hours descending